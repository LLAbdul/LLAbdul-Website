...
**Goal:** Overhaul the About page to be exceptionally clean, apply the Unified Sleek aesthetic to all Admin and edge-case pages, and ensure the project builds without errors for production.

**Architecture:** Utilize `bg-white/5` glassmorphism cards, `Oswald` headers, and `#030509` background consistently. Rebuild the About page as a minimalist, high-end profile. Validate with `next build`.

**Tech Stack:** Next.js 15 App Router, Tailwind CSS v4, Lucide React.

---

### Task 1: Redesign About Page

**Files:**
- Modify: `src/app/about/page.tsx`

- [ ] **Step 1: Rewrite About page for a cleaner, premium look**

The About page needs a cleaner, more focused layout. We'll use a massive cinematic header, a central glass card for stats, and clean typography for the story, removing cluttered grids.
...
git commit -m "feat: redesign about page to cleaner premium layout"
```

### Task 2: Re-theme Admin Pages
...
- [ ] **Step 1: Admin Layout & Dashboard**

Update `src/app/admin/layout.tsx`, `src/app/admin/page.tsx`, and `src/app/admin/login/page.tsx` using a task subagent. Replace any solid `#111833` or `#1A2340` colors with `bg-white/5 backdrop-blur-xl border-white/10`. Headings should use `font-serif text-white`. Buttons use `#C9082A` or `#FFD700`.

- [ ] **Step 2: Commit**

```bash
git add src/app/admin/layout.tsx src/app/admin/page.tsx src/app/admin/login/page.tsx
git commit -m "feat: apply unified sleek theme to admin dashboard and login"
```

### Task 3: Re-theme Matchup Editor Forms

**Files:**
- Modify: `src/app/admin/matchups/new/page.tsx`
- Modify: `src/app/admin/matchups/[id]/edit/page.tsx`
- Modify: `src/app/admin/matchups/page.tsx`

- [ ] **Step 1: Update Matchup Forms**

Update the forms and lists in `src/app/admin/matchups/*`. 
- Input fields: `bg-white/5 border border-white/10 text-white focus:border-[#C9082A] focus:ring-1 focus:ring-[#C9082A]`.
- Cards: `bg-white/5 border-white/10`.
- Use `font-serif text-white` for headers.

- [ ] **Step 2: Commit**

```bash
git add src/app/admin/matchups/
git commit -m "feat: apply unified sleek theme to admin matchup forms"
```

### Task 4: Fix Edge Cases (Error, Not Found, Loading)

**Files:**
- Modify: `src/app/error.tsx`
- Modify: `src/app/not-found.tsx`
- Modify: `src/app/loading.tsx`
- Modify: `src/app/matchups/loading.tsx`
- Modify: `src/app/matchups/[enemyChampion]/loading.tsx`

- [ ] **Step 1: Update error and loading states**

Update these files to remove old purples/blues.
- Use `bg-[#030509]` where full backgrounds are needed.
- Use `font-serif text-white` for 404/Error headings.
- Skeletons in `loading.tsx` should use `bg-white/5 animate-pulse rounded-xl`.

- [ ] **Step 2: Commit**

```bash
git add src/app/error.tsx src/app/not-found.tsx src/app/loading.tsx src/app/matchups/
git commit -m "feat: apply unified sleek theme to error and loading states"
```
...
If the build fails due to typescript errors, unused variables, or Next.js config errors, fix them and re-run build.
If the build succeeds, the site is production ready.
